...
class OpenTok(object):
    ...
    def generate_token(self, session_id, role=Roles.publisher, expire_time=None, data=None):
        ...
        sub_session_id_bytes_padded = sub_session_id_bytes + ('=' * (-len(sub_session_id_bytes) % 4))
        try:
            decoded_session_id = base64.b64decode(sub_session_id_bytes_padded, '-_')
        ...
        decoded_base64_bytes = 'partner_id={api_key}&sig={sig}:{payload}'.format(
            api_key = self.api_key,
            sig     = sig,
            payload = data_string
        )
        token = '{sentinal}{base64_data}'.format(
    ...
    def create_session(self, location=None, media_mode=MediaModes.relayed):
        ...
        try:
            ...
            response = urlfetch.fetch(url=self.session_url(),
                payload=options,
